				...
				try {
					...
					try {
						...
						try {
							...
							try {
								// 默认的Date 输入格式
								SimpleDateFormat sdf = new SimpleDateFormat("yyyy年MM月dd日 HH点mm分ss秒", locale);
								return new Timestamp(sdf.parse(arg).getTime());
							} catch (Exception e5) {
								try {
									// 默认的Date 输入格式
									SimpleDateFormat sdf = new SimpleDateFormat("yyyy年MM月dd日", locale);
									return new Timestamp(sdf.parse(arg).getTime());
								} catch (Exception e6) {
									// throw new
									...
									try {
										// 默认的Date 输入格式
										SimpleDateFormat sdf = new SimpleDateFormat("yyyy年MM月", locale);
										return new Timestamp(sdf.parse(arg).getTime());
									} catch (Exception e7) {
										e.printStackTrace();
										return null;
