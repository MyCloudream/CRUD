...
public class VideoDaoImpl extends BaseDao<Video> implements VideoDao{
	...
	@Override
	public Integer add(Video entity) {
		try {
	...
	@Override
	public boolean upd(Video entity) {
		try {
	...
	public boolean del(Integer id) {
		try {
			super.deleteEntity(new Video(id));
			return true;
		} catch (Exception e) {
	...
	public Video findById(Integer id) {
		try {
			return super.get(Video.class, id);
		} catch (Exception e) {
			logger.error("dao≤È’“¥ÌŒÛ", e);
			return null;
		}
	...
	public Map<String, Object> findVideoSearch(Map<String, Object> map, EasyUiPager easyUiPager) {
		try {
			/*
			 * String select =
			 * "select new Video(id,title,type,subdate,upddate) ";
			 */
			...
			List<Video> list = super.findHqlByPager(/* select + */Hql + orderby, (easyUiPager.getPage() - 1) * easyUiPager.getRows(), easyUiPager.getRows());
