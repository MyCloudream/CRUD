...
(function(){
	if($.Datatype){
		...
		$.extend($.Datatype,{
			 ...
			 * 时间格式可以是：10:01:10 / 02:10 如 2012-01-02 02:10 2012-01-02
			 */
			...
			"unequal":function(gets,obj,curform,datatype){
				 ...
				 * eg1. <input type="text" name="test" datatype="unequal"
				 * with="name" /> eg2. <input type="text" name="test"
				 * datatype="unequal" with=".name" /> eg3. <input type="text"
				 * name="test" datatype="unequal" with="#name" />
				 * 
				 * 也可以用来验证不能与with指定的值相等 当上面根据class，id和name都查找不到对象时，会直接跟with的值比较
				 * eg4. <input type="text" name="test" datatype="num unequal"
				 * with="100" /> 该文本框的值不能等于100
				 */
			...
			"notvalued":function(gets,obj,curform,datatype){
				 ...
				 * 注意需要给表单元素绑定with属性来指定要比较的表单元素，可以是clas，id或者是name属性值 <input
				 * type="text" name="username" id="name" class="name" /> eg.
				 * <input type="password" name="test" datatype="notvalued"
				 * with=".name" />
				 * 
				 * 也可以用来验证不能包含with指定的值 当上面根据class，id和name都查找不到对象时，会直接跟with的值比较
				 * eg2. <input type="password" name="test" datatype="notvalued"
				 * with="validform" /> 要求不能含有"validform"字符
				 */
				var withele=$.trim(obj.attr("with"));
				var val=curform.find(withele+",[name='"+withele+"']").val() || withele;
			...
			"min":function(gets,obj,curform,datatype){
				/*
				 * checkbox最少选择n项 注意需要给表单元素绑定min属性来指定是至少需要选择几项，没有绑定的话使用默认值 eg.
				 * <input type="checkbox" name="test" datatype="min" min="3" />
				 */
			...
			"max":function(gets,obj,curform,datatype){
				/*
				 * checkbox最多选择n项 注意需要给表单元素绑定max属性来指定是最多需要选择几项，没有绑定的话使用默认值 eg.
				 * <input type="checkbox" name="test" datatype="max" max="3" />
				 */
			...
			"numrange":function(gets,obj,curform,datatype){
				/*
				 * 判断数值范围 注意需要给表单元素绑定max,min属性来指定最大或最小可输入的值，没有绑定的话使用默认值
				 */
			...
			"daterange":function(gets,obj,curform,datatype){
				/*
				 * 判断日期范围 注意需要给表单元素绑定max或min属性，或两个同时绑定来指定最大或最小可输入的日期
