	...
	$(function() {
		$.ajax({
			...
			success : function(data) {
				...
				$("#type").append(str);
				$("select[name='tid'] option[value='" + formdata[0].tid + "']").attr("selected",true);
			}
		});
		valform("id", formdata[0].id, "");
		valform("name", formdata[0].name, "");
		valform("no", formdata[0].no, "");
	});
</script>
</head>
<body>
	<form class="formobj" method="post" action="/busi/MaterialSuffixManagerUpd.do">
		<table class="vtb">
			<tr>
				<th>
					<span class="t_t">*</span>
